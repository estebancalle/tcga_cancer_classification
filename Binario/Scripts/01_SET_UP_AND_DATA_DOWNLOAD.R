...
if (!dir.exists(here("Data"))) {
  dir.create(here("Data", "All_datasets", "Tumor", "csv"), recursive = TRUE)
  dir.create(here("Data", "All_datasets", "Tumor", "rda"), recursive = TRUE)
  dir.create(here("Data", "All_datasets", "Normal", "csv"), recursive = TRUE)
  dir.create(here("Data", "All_datasets", "Normal", "rda"), recursive = TRUE)
  dir.create(here("Data", "All_datasets", "SE"), recursive = TRUE)
...
download_and_prepare <- function(project) {
  ...
  ## seleccionamos muestras normales  y las guardamos
 
  
  # Transponemos Matriz para que los genes sean las columnas. Incluimos columna con la etiqueta del proyecto.
  Matrixt <- Matrix[,samplesNT] %>% t() %>% as_tibble(rownames = NA) %>% 
    rownames_to_column() %>% rename(Sample = rowname) %>% 
    add_column(Labels = project, .after = 1)
  
  # Exportamos en csv la matriz transpuesta
  export(Matrixt, here("Data", "All_datasets","Normal", "csv", glue("{project}_Matrixt.csv")))
...
# Almacenamos en variable los archivos summarized experiment .rda guardados en la carpeta principal
archivos <- list.files(path = here(), pattern = "\\.rda$")

# Mover los archivos . rds a la subcarpeta "data"
for (archivo in archivos) {
  file.rename(here(archivo), here("Data/All_datasets/SE/", archivo))
}
...
# Obtener la lista de todos los archivos CSV de tumor en la ruta especificada.
# Leer cada archivo CSV con map.
#Combinar todos los objetos en un solo conjunto de datos con bind rows.
ruta_csvs <- here("Data/All_datasets/Tumor/csv/")

dataset_tumor_unificado <- list.files(path = ruta_csvs, pattern = "\\.csv") %>%
  map( ~ rio::import(file.path(ruta_csvs, .), setclass = "data.frame")) %>%
  bind_rows() %>% add_column(Class = "Tumor", .after = 2)

# Muestreo 650 muestras aleatorias de cada factor sin reemplazamiento y convertir el resultado en un dataframe
set.seed(1993)
df_unficado_tumor_sampled <- dataset_tumor_unificado %>%
  group_by() %>%
  sample_n(size = 650, replace = FALSE) %>%
  as.data.frame()


# Exportamos el csv  combinado
export(dataset_tumor_unificado, here("Data/Merged_dataset/Preprocessed","dataset_tumor_unificado.csv"))

# Para guardar el objeto en formato .rds
saveRDS(dataset_tumor_unificado, here("Data/Merged_dataset/Preprocessed","dataset_tumor_unificado.rds"))

# Exportamos el csv  combinado del df_sampled
export(df_unficado_tumor_sampled, here("Data/Merged_dataset/Preprocessed","df_unficado_tumor_sampled.csv"))

# Para guardar el objeto en formato .rds del df_sampled
saveRDS(df_unficado_tumor_sampled, here("Data/Merged_dataset/Preprocessed","df_unficado_tumor_sampled.rds"))



# Obtener la lista de todos los archivos CSV de normal en la ruta especificada.
# Leer cada archivo CSV con map.
#Combinar todos los objetos en un solo conjunto de datos con bind rows.
ruta_csvs <- here("Data/All_datasets/Normal/csv/")

dataset_normal_unificado <- list.files(path = ruta_csvs, pattern = "\\.csv") %>%
  map( ~ rio::import(file.path(ruta_csvs, .), setclass = "data.frame")) %>%
  bind_rows() %>% add_column(Class = "Normal", .after = 2)

dim(dataset_normal_unificado)

# Las muestras de muestras normales son escasas, en este caso son 630.  Avanzamos con el dataset entero normal sin sampleo.

# # Muestreo 650 muestras aleatorias de cada factor sin reemplazamiento y convertir el resultado en un dataframe
# set.seed(1993)
# df_unficado_normal_sampled <- dataset_normal_unificado %>%
#   group_by() %>%
#   sample_n(size = 650, replace = FALSE) %>%
#   as.data.frame()
...
# Exportamos el csv  combinado
export(dataset_normal_unificado, here("Data/Merged_dataset/Preprocessed","dataset_normal_unificado.csv"))

# Para guardar el objeto en formato .rds
saveRDS(dataset_normal_unificado, here("Data/Merged_dataset/Preprocessed","dataset_normal_unificado.rds"))



# Combinamos data set tumor sampleado y el data set de muestras normales completo.


df_tumor_and_normal <- rbind(df_unficado_tumor_sampled, dataset_normal_unificado)

# Exportamos el csv  combinado
export(df_tumor_and_normal, here("Data/Merged_dataset/Preprocessed","df_tumor_and_normal.csv"))

# Para guardar el objeto en formato .rds
saveRDS(df_tumor_and_normal, here("Data/Merged_dataset/Preprocessed","df_tumor_and_normal.rds"))
