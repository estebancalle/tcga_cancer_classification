...
metricas_resamples %>%
  ...
  annotate(geom = "text", y = 0.38, x = 19.5, label = "Accuracy basal") +
...
metricas_resamples %>%
  ...
  ggplot(aes(x = reorder(modelo, media), y = media, label = round(media, 2))) +
    ...
    annotate(geom = "text", y = 0.38, x = 19, label = "Accuracy basal") +
...
# Tabla con los modelos y los accuracys  test
print(metricas_predicciones <- metricas_resamples %>% 
        group_by(modelo, metrica) %>% 
        summarise(media = mean(valor)) %>%
        spread(key = metrica, value = media) %>%
        dplyr::select(roc_validacion = ROC) %>%
...
metricas_predicciones %>%
  gather(key = "datos", value = "ROC", -modelo) %>%
  ggplot(aes(
    x = fct_reorder(modelo, ROC),
    y = ROC,
    color = datos,
    label = round(ROC, 2)
  )) +
  ...
  annotate(geom = "text", y = 0.38, x = 19.5, label = "Accuracy basal") + 
  coord_flip() +
  labs(title = "ROC de entrenamiento y test",
       x = "Modelo") +
  theme_bw() +
...
# Extraemos probabilidades de los modelos. Las teníamos ya guardadas. Las cargamos.

prob_knn_boruta <- readRDS(here("Models/Predictions/prob_knn_boruta.rds")) %>% add_column(Modelo = "KNN_BOR", .before = 1)
prob_knn_lasso <- readRDS(here("Models/Predictions/prob_knn_lasso.rds")) %>% add_column(Modelo = "KNN_LASSO", .before = 1)
prob_knn_pca <- readRDS(here("Models/Predictions/prob_knn_pca.rds")) %>% add_column(Modelo = "KNN_PCA", .before = 1)
prob_knn_SPLSDA <- readRDS(here("Models/Predictions/prob_knn_splsda.rds") ) %>% add_column(Modelo = "KNN_SPLSDA", .before = 1)

prob_svml_boruta <- readRDS(here("Models/Predictions/prob_svml_boruta.rds")) %>% add_column(Modelo = "SVML_BOR", .before = 1)
prob_svml_lasso <- readRDS(here("Models/Predictions/prob_svml_lasso.rds")) %>% add_column(Modelo = "SVML_LASSO", .before = 1)
prob_svml_pca <- readRDS(here("Models/Predictions/prob_svml_pca.rds")) %>% add_column(Modelo = "SVML_PCA", .before = 1)
prob_svml_SPLSDA <- readRDS(here("Models/Predictions/prob_svml_splsda.rds")) %>% add_column(Modelo = "SVML_SPLSDA", .before = 1)

prob_svmr_boruta <- readRDS(here("Models/Predictions/prob_svmr_boruta.rds"))  %>% add_column(Modelo = "SVMR_BOR", .before = 1)
prob_svmr_lasso <- readRDS(here("Models/Predictions/prob_svmr_lasso.rds")) %>% add_column(Modelo = "SVMR_LASSO", .before = 1)
prob_svmr_pca <- readRDS(here("Models/Predictions/prob_svmr_pca.rds")) %>% add_column(Modelo = "SVMR_PCA", .before = 1)
prob_svmr_SPLSDA <- readRDS(here("Models/Predictions/prob_svmr_splsda.rds")) %>% add_column(Modelo = "SVMR_SPLSDA", .before = 1)

prob_nb_boruta <- readRDS(here("Models/Predictions/prob_nb_boruta.rds")) %>% add_column(Modelo = "NB_BOR", .before = 1)
prob_nb_lasso <- readRDS(here("Models/Predictions/prob_nb_lasso.rds")) %>% add_column(Modelo = "NB_LASSO", .before = 1)
prob_nb_pca <- readRDS(here("Models/Predictions/prob_nb_pca.rds")) %>% add_column(Modelo = "NB_PCA", .before = 1)
prob_nb_SPLSDA <- readRDS(here("Models/Predictions/prob_nb_splsda.rds")) %>% add_column(Modelo = "NB_SPLSDA", .before = 1)

prob_rf_boruta <- readRDS(here("Models/Predictions/prob_rf_boruta.rds")) %>% add_column(Modelo = "RF_BOR", .before = 1)
prob_rf_lasso <- readRDS(here("Models/Predictions/prob_rf_lasso.rds")) %>% add_column(Modelo = "RF_LASSO", .before = 1)
prob_rf_pca <- readRDS(here("Models/Predictions/prob_rf_pca.rds")) %>% add_column(Modelo = "RF_PCA", .before = 1)
prob_rf_SPLSDA <- readRDS(here("Models/Predictions/prob_rf_splsda.rds")) %>% add_column(Modelo = "RF_SPLSDA", .before = 1)

prob_ann_boruta <- readRDS(here("Models/Predictions/probs_ann_boruta.rds"))
prob_ann_lasso <- readRDS(here("Models/Predictions/probs_ann_lasso.rds"))
prob_ann_pca <- readRDS(here("Models/Predictions/probs_ann_pca.rds"))
prob_ann_SPLSDA <- readRDS(here("Models/Predictions/probs_ann_splsda.rds"))
#prob_msgl_lasso <- readRDS(here("Models/Predictions/prob_lasso_lasso.rds"))

colnames(prob_ann_boruta) <- colnames(prob_rf_boruta)[2:3]
colnames(prob_ann_lasso) <- colnames(prob_rf_boruta)[2:3]
colnames(prob_ann_pca) <- colnames(prob_rf_boruta)[2:3]
colnames(prob_ann_SPLSDA) <- colnames(prob_rf_boruta)[2:3]


prob_ann_boruta <- prob_ann_boruta %>% as.data.frame() %>%  add_column(Modelo = "ANN_BOR", .before = 1)
prob_ann_lasso <- prob_ann_lasso %>% as.data.frame() %>%  add_column(Modelo = "ANN_LASSO", .before = 1)
prob_ann_pca <- prob_ann_pca %>% as.data.frame() %>%  add_column(Modelo = "ANN_PCA", .before = 1)
prob_ann_SPLSDA <- prob_ann_SPLSDA %>% as.data.frame() %>%   add_column(Modelo = "ANN_SPLSDA", .before = 1)



# Unificamos Probabilidades y predicciones


#df_knn <- df_roc %>% dplyr::filter(Modelo == "ANN_BOR") %>% mutate(ROC = roc_curve(truth = data$Prediccion))


KNN_BOR  <-  cbind(Clase = pred_knn_boruta, prob_knn_boruta) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "KNN_BOR")
KNN_LASSO <-  cbind(Clase = pred_knn_lasso, prob_knn_lasso) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "KNN_LASSO")
KNN_PCA  <-  cbind(Clase = pred_knn_pca, prob_knn_pca) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "KNN_PCA")
KNN_SPLSDA <- cbind(Clase = pred_knn_SPLSDA, prob_knn_SPLSDA) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "KNN_SPLSDA")
ANN_BOR  <-  cbind(Clase = pred_ann_boruta, prob_ann_boruta) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "ANN_BOR")
ANN_LASSO  <-  cbind(Clase = pred_ann_lasso, prob_ann_lasso) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "ANN_LASSO")
ANN_PCA  <-  cbind(Clase = pred_ann_pca, prob_ann_pca) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "ANN_PCA")
ANN_SPLSDA  <-  cbind(Clase = pred_ann_SPLSDA, prob_ann_SPLSDA) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "ANN_SPLSDA")
SVML_BOR  <-  cbind(Clase = pred_svml_boruta, prob_svml_boruta) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "SVML_BOR")
SVML_LASSO  <-  cbind(Clase = pred_svml_lasso, prob_svml_lasso) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "SVML_LASSO")
SVML_PCA  <-  cbind(Clase = pred_svml_pca, prob_svml_pca) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "SVML_PCA")
SVML_SPLSDA  <-  cbind(Clase = pred_svml_SPLSDA, prob_svml_SPLSDA) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "SVML_SPLSDA")
SVMR_BOR  <-  cbind(Clase = pred_svmr_boruta, prob_svmr_boruta) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "SVMR_BOR")
SVMR_LASSO  <-  cbind(Clase = pred_svmr_lasso, prob_svmr_lasso) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "SVMR_LASSO")
SVMR_PCA  <-  cbind(Clase = pred_svmr_pca, prob_svmr_pca) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "SVMR_PCA")
SVMR_SPLSDA  <- cbind(Clase = pred_svmr_SPLSDA,  prob_svmr_SPLSDA) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "SVMR_SPLSDA")
NB_BOR  <- cbind(Clase = pred_nb_boruta,  prob_nb_boruta) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "NB_BOR")
NB_LASSO  <- cbind(Clase = pred_nb_lasso,  prob_nb_lasso) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "NB_LASSO")
NB_PCA  <-  cbind(Clase = pred_nb_pca, prob_nb_pca) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "NB_PCA")
NB_SPLSDA  <-cbind(Clase = pred_nb_SPLSDA, prob_nb_SPLSDA) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "NB_SPLSDA")
RF_BOR  <-  cbind(Clase = pred_rf_boruta, prob_rf_boruta) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "RF_BOR")
RF_LASSO  <- cbind(Clase = pred_rf_lasso,  prob_rf_lasso) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "RF_LASSO")
RF_PCA  <- cbind(Clase = pred_rf_pca,  prob_rf_pca) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "RF_PCA")
RF_SPLSDA  <-  cbind(Clase = pred_rf_SPLSDA, prob_rf_SPLSDA) %>% roc_curve(truth = Clase, Normal) %>% add_column(Modelo= "RF_SPLSDA")


rocdfs <- rbind(KNN_BOR, KNN_LASSO, KNN_PCA, KNN_SPLSDA, ANN_BOR, ANN_LASSO, ANN_PCA, ANN_SPLSDA, SVML_BOR, SVML_LASSO, SVML_PCA, SVML_SPLSDA, SVMR_BOR, SVMR_LASSO, SVMR_PCA, SVMR_SPLSDA, NB_BOR, NB_LASSO, NB_PCA, NB_SPLSDA, RF_BOR, RF_LASSO, RF_PCA, RF_SPLSDA)
...
rocdfs %>% 
  ...
  labs(
    ...
    colour = "Modelos",
    title = "Curvas ROC de cada modelo con los datos test"
  )

# Gráfico comparación métricas modelos
ggsave(here("Plots", "ROC_plot.png") , width = 7.69, height = 8.25)





KNN_BOR  <-  cbind(Clase = pred_knn_boruta, prob_knn_boruta) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "KNN_BOR")
KNN_LASSO <-  cbind(Clase = pred_knn_lasso, prob_knn_lasso) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "KNN_LASSO")
KNN_PCA  <-  cbind(Clase = pred_knn_pca, prob_knn_pca) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "KNN_PCA")
KNN_SPLSDA <- cbind(Clase = pred_knn_SPLSDA, prob_knn_SPLSDA) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "KNN_SPLSDA")
ANN_BOR  <-  cbind(Clase = pred_ann_boruta, prob_ann_boruta) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "ANN_BOR")
ANN_LASSO  <-  cbind(Clase = pred_ann_lasso, prob_ann_lasso) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "ANN_LASSO")
ANN_PCA  <-  cbind(Clase = pred_ann_pca, prob_ann_pca) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3)%>% add_column(Modelo= "ANN_PCA")
ANN_SPLSDA  <-  cbind(Clase = pred_ann_SPLSDA, prob_ann_SPLSDA) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "ANN_SPLSDA")
SVML_BOR  <-  cbind(Clase = pred_svml_boruta, prob_svml_boruta) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "SVML_BOR")
SVML_LASSO  <-  cbind(Clase = pred_svml_lasso, prob_svml_lasso) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "SVML_LASSO")
SVML_PCA  <-  cbind(Clase = pred_svml_pca, prob_svml_pca) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "SVML_PCA")
SVML_SPLSDA  <-  cbind(Clase = pred_svml_SPLSDA, prob_svml_SPLSDA) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "SVML_SPLSDA")
SVMR_BOR  <-  cbind(Clase = pred_svmr_boruta, prob_svmr_boruta) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "SVMR_BOR")
SVMR_LASSO  <-  cbind(Clase = pred_svmr_lasso, prob_svmr_lasso) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "SVMR_LASSO")
SVMR_PCA  <-  cbind(Clase = pred_svmr_pca, prob_svmr_pca) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "SVMR_PCA")
SVMR_SPLSDA  <- cbind(Clase = pred_svmr_SPLSDA,  prob_svmr_SPLSDA) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "SVMR_SPLSDA")
NB_BOR  <- cbind(Clase = pred_nb_boruta,  prob_nb_boruta) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3 )%>% add_column(Modelo= "NB_BOR")
NB_LASSO  <- cbind(Clase = pred_nb_lasso,  prob_nb_lasso) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "NB_LASSO")
NB_PCA  <-  cbind(Clase = pred_nb_pca, prob_nb_pca) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "NB_PCA")
NB_SPLSDA  <-cbind(Clase = pred_nb_SPLSDA, prob_nb_SPLSDA) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "NB_SPLSDA")
RF_BOR  <-  cbind(Clase = pred_rf_boruta, prob_rf_boruta) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "RF_BOR")
RF_LASSO  <- cbind(Clase = pred_rf_lasso,  prob_rf_lasso) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "RF_LASSO")
RF_PCA  <- cbind(Clase = pred_rf_pca,  prob_rf_pca) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3) %>% add_column(Modelo= "RF_PCA")
RF_SPLSDA  <-  cbind(Clase = pred_rf_SPLSDA, prob_rf_SPLSDA) %>% roc_auc(truth = Clase, Normal) %>% select(.estimate) %>% round(3)%>% add_column(Modelo= "RF_SPLSDA")




rocdfs <- rbind(KNN_BOR, KNN_LASSO, KNN_PCA, KNN_SPLSDA, ANN_BOR, ANN_LASSO, ANN_PCA, ANN_SPLSDA, SVML_BOR, SVML_LASSO, SVML_PCA, SVML_SPLSDA, SVMR_BOR, SVMR_LASSO, SVMR_PCA, SVMR_SPLSDA, NB_BOR, NB_LASSO, NB_PCA, NB_SPLSDA, RF_BOR, RF_LASSO, RF_PCA, RF_SPLSDA) %>% arrange(Modelo)


df <-  df %>% arrange(Modelo)
dffinal <- cbind(df[-9,], rocdfs[1]) %>%  rename(AUC = .estimate) %>% arrange(desc(Accuracy))


# Convertir el DataFrame en una tabla con kable()
tabla <- kable(dffinal, caption = "Tabla comparativa de las distintas métricas de modelos con los datos tests")
...
grid.table(dffinal)
dev.off()
export(dffinal, (here("Tables/compatativa_modelos.csv")))
...
dffinal %>%
  ...
  gather(key = "medida", value = "valor",-Modelo) %>%
  filter(
    str_detect(Modelo, "SVMR_BOR") |
      str_detect(Modelo, "RF_SPLSDA") |
      str_detect(Modelo, "SVMR_SPLSDA")
  ) %>%
  ggplot(data = ., aes(x = medida, y = valor, color = Modelo)) +
  geom_point(size = 4) +
  geom_line(aes(group = Modelo)) +
  scale_color_manual(values = brewer.pal(n = 9, name = "Set1")) +
  labs(title = "Comparación de métricas de los modelos Ensemble (RF Y SVMR)",
       subtitle = "Para las distintas técnicas de Feature Selection") +
  theme_test() +
  theme(legend.position = "bottom")

ggsave(here("Plots", glue(
  "SVMR_RF_BEST_Model_metrics_by_fs_plot.png"
...
dffinal %>%
  ...
  gather(key = "medida", value = "valor",-Modelo) %>%
  filter(
    str_detect(Modelo, "SVMR_") |
      str_detect(Modelo, "RF_") 
...
ggsave(here("Plots", glue(
  "SVMR_RF_Model_metrics_by_fs_plot.png"
)), width = 7.69, height = 8.25)
...
# plots Matriz confusión mejores modelos



plot_conf_svmr_boruta <-
    confusionMatrixPlot(MatrizConfusion = conf_svmr_boruta, " SVM Radial con Boruta")

ggsave(here("Plots", "plot_conf_svmr_boruta.png"))


plot_conf_svmr_SPLSDA <-
    confusionMatrixPlot(MatrizConfusion = conf_svmr_SPLSDA, " SVM Radial con SPLSDA")
...
plot_conf_rf_SPLSDA <-
  confusionMatrixPlot(MatrizConfusion = conf_rf_SPLSDA, " Random forest con SPLSDA")

ggsave(here("Plots", "plot_conf_rf_lasso.png"))

plot_conf_svml_boruta <-
  confusionMatrixPlot(MatrizConfusion = conf_svml_boruta, " SVM Lineal con Boruta")

ggsave(here("Plots", "plot_conf_svml_boruta.png"))
...
nombres_modelos <- predicciones  %>% dplyr::select(SVMR_BOR, RF_SPLSDA, SVMR_SPLSDA) %>% colnames()
