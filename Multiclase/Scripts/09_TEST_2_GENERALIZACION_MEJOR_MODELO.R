...
df_unificado <- readRDS(here("Data/Merged_dataset/Preprocessed","dataset_unificado.rds"))
df_sampleado <- readRDS(here("Data/Merged_dataset/Preprocessed","df_unficado_sampled.rds"))
...
df_test2_generalizacion <-  df_2_test_scaled %>% dplyr::select(Sample, all_of(names(datos_test_boruta)))
...
# Predicciones
pred_rf_boruta <- predict(modelo_rf_boruta,newdata = df_test2_generalizacion[,-2], type = "raw")
# Predicciones
probs_rf_boruta <- predict(modelo_rf_boruta,newdata = df_test2_generalizacion[,-2], type = "prob")


# Matriz confusión
(conf_rf_boruta<- confusionMatrix(pred_rf_boruta, as.factor(df_test2_generalizacion$Labels)))


# Gráfico matriz de confusión
plot_conf_rf_boruta <-
  # funcion  definida en el script 8, paraplot gráfico
  confusionMatrixPlot(MatrizConfusion = conf_rf_boruta, " Random forest con Boruta (Test generalización 2)")

ggsave(here("Plots", "plot_conf_rf_boruta_test2.png"))
