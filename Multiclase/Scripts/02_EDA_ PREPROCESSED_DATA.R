...
df_unficado_sampled <- readRDS(here("Data/Merged_dataset/Preprocessed","df_unficado_sampled.rds"))
...
message(
  glue(
    "El dataset combinado preprocesado tiene {dim(datos_preprocessed)[1]} filas y {dim(datos_preprocessed)[2]} columnas."
  )
)

#Cambiamos la variable Labels a factor en ambos datasets
datos_preprocessed$Labels <- as.factor(datos_preprocessed$Labels)
...
if (any(!complete.cases(datos_preprocessed))) {
...
tabla_num_tumor_sampled <-
  df_unficado_sampled %>% dplyr::select(Labels) %>%
  mutate(Labels = factor(Labels)) %>%
  group_by(Labels) %>%
  dplyr::count()
...
plot_num_tumor <- ggplot(tabla_num_tumor_sampled, aes(x = reorder(Labels, n), y = n, fill = Labels)) +
  geom_col() +
  scale_fill_discrete(name = "Tipo de tumor") +
  coord_flip() +
  theme_bw() +
  labs(x = "Tipo de tumor", y = "Número de muestras",
       title = "Número de muestras por tipo de tumor",
       subtitle = "Entrenamiento algoritmos") +
  ggeasy::easy_remove_legend()


# Guardar la imagen en la carpeta "plots"
ggsave(here("plots", "plot_num_tumor_sampled.png"))
