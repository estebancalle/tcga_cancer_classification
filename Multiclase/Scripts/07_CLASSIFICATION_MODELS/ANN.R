...
mod_keras %>% 
  layer_dense(units = 50, activation = "relu",input_shape =c(dim(matrix_datos_train)[2])) %>% 
...
return(list(mod_nnet = mod_nnet, y_pred.f = y_pred.f, y_test.f = y_test.f))
}
...
(conf_ann_boruta <- confusionMatrix(y_predf_boruta,y_test.f))
...
(conf_ann_pca <- confusionMatrix(y_predf_pca,y_test.f))
...
(conf_ann_lasso <- confusionMatrix(y_predf_lasso,y_test.f))
...
# Matriz de confusion Ann lasso
(conf_ann_splsda <- confusionMatrix(y_predf_splsda,y_test.f))
